...
stopifnot(all.equal( sqrt(anova(m.testinter.col)[1,4]*2/64),
           res.inter$fixed$dprimeav[1, "Colourbalance"], tol = 1e-4))
stopifnot(all.equal( sqrt(anova(m.testinter.col)[2,4]*2/48),
           res.inter$fixed$dprimeav[2, "Colourbalance"], tol = 1e-4))
...
stopifnot(all.equal( sqrt(sum(deltadifs^2/anova(m.testinter.col)[6,3])/(66)), 
           res.inter$fixed$dprimeav[3, "Colourbalance"], tol = 1e-4))
